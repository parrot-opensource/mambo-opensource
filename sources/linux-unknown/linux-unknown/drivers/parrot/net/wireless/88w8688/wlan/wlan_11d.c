...
/*find the NoOfChan-th chan after the firstChan*/
{
    ...
    if (band & (BAND_B | BAND_G)) {
        ...
        cfp_no = sizeof(channel_freq_power_UN_BG) / sizeof(CHANNEL_FREQ_POWER);
    } else {
        if (band & BAND_A) {
                           ...
                           parsed_region_chan_11d_t * parsed_region_chan)
{
    ...
    for (j = 0, lastChan = 0; j < NoOfSubband; j++) {
        ...
        for (i = 0; idx < MAX_NO_OF_CHAN && i < NoOfChan; i++) {
            ...
            /* step5: We don't need to Check if curChan is supported by mrvl in 
               region */
...
wlan_set_domain_info_11d(wlan_private * priv)
{
    ...
    if (ret)
        PRINTM(INFO, "11D: Fail to download domain Info\n");
...
wlan_set_universaltable(wlan_private * priv, u8 band)
{
    ...
    if (band & (BAND_B | BAND_G)) {
        ...
        Adapter->universal_channel[i].Band = (band & BAND_G) ? BAND_G : BAND_B;
        i++;
    }
...
wlan_cmd_enable_11d(wlan_private * priv, struct iwreq *wrq)
{
    ...
    case CMD_ENABLED:
        ...
        Adapter->State11D.userEnable11D = ENABLE_11D;
        break;
    case CMD_DISABLED:
...
wlan_ret_802_11d_domain_info(wlan_private * priv, HostCmd_DS_COMMAND * resp)
{
    ...
    if (NoOfSubband > MRVDRV_MAX_SUBBAND_802_11D) {
        PRINTM(INFO, "Invalid Number of Sub band returned!!\n");
        return WLAN_STATUS_FAILURE;
    }
...
wlan_parse_dnld_countryinfo_11d(wlan_private * priv)
{
    ...
    if (wlan_get_state_11d(priv) == ENABLE_11D) {
        ...
        ret =
            wlan_parse_domain_info_11d(&Adapter->pAttemptedBSSDesc->CountryInfo,
...
wlan_create_dnld_countryinfo_11d(wlan_private * priv, u8 band)
{
    ...
    /* update parsed_region_chan_11; download domaininf to FW */
