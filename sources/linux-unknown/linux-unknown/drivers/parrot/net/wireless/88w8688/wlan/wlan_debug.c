...
static struct debug_data items[] = {
    ...
    {"wmmPkts", item_size(wmm.packetsQueued), 0, item_addr(wmm.packetsQueued)},
    ...
    {"num_event_chan_switch", item_dbg_size(num_event_channel_switch), 0,
     item_dbg_addr(num_event_channel_switch)},
    ...
    {"cmd_sent", item1_size(cmd_sent), 0, item1_addr(cmd_sent)},
    {"data_sent", item1_size(data_sent), 0, item1_addr(data_sent)},
};
 ...
 */
static int
wlan_debug_read(char *page, char **s, off_t off, int cnt, int *eof, void *data)
{
    int val = 0;
    char *p = page;
    int i;
 ...
 */
static int
wlan_debug_write(struct file *f, const char *buf, unsigned long cnt, void *data)
{
    int r, i;
    char *pdata;
    char *p;
