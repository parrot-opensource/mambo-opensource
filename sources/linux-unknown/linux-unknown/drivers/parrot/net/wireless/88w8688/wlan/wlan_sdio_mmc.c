 ...
 */
int
wlan_sdio_suspend(struct device *dev)
{
    struct sdio_func *func = dev_to_sdio_func(dev);
    mmc_pm_flag_t pm_flags = 0;
    struct sdio_mmc_card *cardp = NULL;
    ...
    if (func) {
        ...
        PRINTM(CMND, "%s: suspend: PM flags = 0x%x\n", sdio_func_id(func),
               pm_flags);
        ...
        if (!cardp || !cardp->priv) {
            PRINTM(ERROR, "Card or private structure is not valid\n");
            LEAVE();
            return ret;
        }
    } else {
    ...
    /* Enable Host Sleep */
    if (pm_keep_power) {
        if (priv->adapter->HSCfg.conditions != HOST_SLEEP_CFG_CANCEL)
            wlan_enable_hs(priv);

        if (priv->adapter->HS_Activated == TRUE) {
#ifdef MMC_PM_SKIP_RESUME_PROBE
            PRINTM(CMND, "suspend with MMC_PM_KEEP_POWER and "
                   "MMC_PM_SKIP_RESUME_PROBE\n");
            ret = sdio_set_host_pm_flags(func, MMC_PM_KEEP_POWER |
                                         MMC_PM_SKIP_RESUME_PROBE);
#else
            PRINTM(CMND, "suspend with MMC_PM_KEEP_POWER\n");
            ret = sdio_set_host_pm_flags(func, MMC_PM_KEEP_POWER);
#endif /* MMC_PM_SKIP_RESUME_PROBE */
        }
    }
 ...
 */
int
wlan_sdio_resume(struct device *dev)
{
    struct sdio_func *func = dev_to_sdio_func(dev);
    mmc_pm_flag_t pm_flags = 0;
    ...
    if (func) {
        pm_flags = sdio_get_host_pm_caps(func);
        PRINTM(CMND, "%s: resume: PM flags = 0x%x\n", sdio_func_id(func),
               pm_flags);
        cardp = sdio_get_drvdata(func);
        if (!cardp || !cardp->priv) {
            PRINTM(ERROR, "Card or private structure is not valid\n");
...
static struct dev_pm_ops wlan_sdio_pm_ops = {
    .suspend = wlan_sdio_suspend,
    .resume = wlan_sdio_resume,
};
#endif /* MMC_PM_KEEP_POWER */
#endif /* SDIO_SUSPEND_RESUME */
...
#ifdef WAPI_DRIVER
        if (priv->adapter->SecInfo.WAPIEnabled) {
            len = rx_len;
            rx_len = ALLOC_BUF_SIZE;
        }
#endif
