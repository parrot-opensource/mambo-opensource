 ...
 *
 *  @param pACParam Pointer to the AC parameters to display
 *
 *  @return         void
 */
static void
 ...
 *  @param priv     Pointer to the wlan_private driver data struct
 *
 *  @return         a pointer to skb buffer
 */
...
wmm_pop_highest_prio_skb(wlan_private * priv)
{
    ...
    for (i = 0; i < MAX_AC_QUEUES; i++) {
        ...
        if (!list_empty((struct list_head *) &Adapter->wmm.txSkbQ[ac])) {
            ...
            skb = Adapter->wmm.txSkbQ[ac].next;
 ...
 *
 *  @param priv   Pointer to the wlan_private driver data struct
 *  @param acVal  AC to evaluate for downgrading
 *
...
wmm_process_fw_iface_tx_xfer_start(wlan_private * priv)
{
    ...
    atomic_dec(&Adapter->wmm.packetsQueued);
    if (atomic_read(&Adapter->wmm.packetsQueued) <
        WMM_QUEUED_PACKET_LOWER_LIMIT) {
        PRINTM(DATA, "WMM: FW OS+: %d\n",
               atomic_read(&Adapter->wmm.packetsQueued));
        os_start_queue(priv);
    }
}
...
wmm_process_app_iface_tx(wlan_private * priv)
{
    ...
    atomic_inc(&Adapter->wmm.packetsQueued);

    if ((!priv->wlan_dev.data_sent && (Adapter->PSState != PS_STATE_SLEEP))
        || (atomic_read(&Adapter->wmm.packetsQueued) >=
            WMM_QUEUED_PACKET_UPPER_LIMIT)) {
                        ...
                        HostCmd_DS_COMMAND * cmd, void *InfoBuf)
{
    ...
    cmd->Size = wlan_cpu_to_le16(sizeof(HostCmd_DS_WMM_GET_STATUS) + S_DS_GEN);
...
wmm_cleanup_queues(wlan_private * priv)
{
    ...
    atomic_set(&priv->adapter->wmm.packetsQueued, 0);
    ...
    while ((respLen >= sizeof(pTlvHdr->Header)) && valid) {
        ...
        case TLV_TYPE_WMMQSTATUS:
            ...
            PRINTM(INFO, "CMD_RESP: WMM_GET_STATUS: QSTATUS TLV: %d, %d, %d\n",
                   ...
                   pTlvWmmQStatus->FlowRequired, pTlvWmmQStatus->Disabled);
 ...
 */
u8
wmm_compute_driver_packet_delay(wlan_private * priv, const struct sk_buff * skb)
{
    u8 retVal;
    ...
    /* Queue delay is passed as a uint8 in units of 2ms (ms shifted by 1). Min
       value (other than 0) is therefore 2ms, max is 510ms. Pass max value if
       queue_delay is beyond the uint8 range */
...
wmm_process_tx(wlan_private * priv)
{
    ...
    skb = wmm_pop_highest_prio_skb(priv);
    spin_unlock_irqrestore(&Adapter->CurrentTxLock, flags);

    if (skb)
        wlan_process_tx(priv, skb);
...
wlan_wmm_ts_status_ioctl(wlan_private * priv, struct iwreq *wrq)
{
    ...
    if (copy_from_user(&ts_status_ioctl,
                       wrq->u.data.pointer,
                       MIN(wrq->u.data.length, sizeof(ts_status_ioctl))) != 0) {
...
wlan_wmm_addts_req_ioctl(wlan_private * priv, struct iwreq *wrq)
{
    ...
    if (copy_from_user(&addtsIoctl,
        ...
        memcpy(addtsCmd.tspecData, addtsIoctl.tspecData, addtsCmd.tspecDataLen);
        ...
        memcpy(addtsIoctl.tspecData, addtsCmd.tspecData, addtsCmd.tspecDataLen);

        if (copy_to_user(wrq->u.data.pointer, &addtsIoctl, sizeof(addtsIoctl))) {
            PRINTM(INFO, "Copy to user failed\n");
            LEAVE();
            return -EFAULT;
        }
...
wlan_wmm_delts_req_ioctl(wlan_private * priv, struct iwreq *wrq)
{
    ...
    if (copy_from_user(&deltsIoctl,
        ...
        memcpy(deltsCmd.tspecData, deltsIoctl.tspecData, deltsCmd.tspecDataLen);
 ...
 */
int
wlan_cmdresp_wmm_addts_req(wlan_private * priv, const HostCmd_DS_COMMAND * resp)
{
    wlan_cmd_wmm_addts_req_t *pAddTsCmd;
    const HostCmd_DS_WMM_ADDTS_REQ *pCmdResp;

    ENTER();
    if (!priv->adapter->CurCmd->pdata_buf) {
        LEAVE();
    ...
    HEXDUMP("TSPEC: ADDTS data", pAddTsCmd->tspecData, pAddTsCmd->tspecDataLen);
 ...
 */
int
wlan_cmdresp_wmm_delts_req(wlan_private * priv, const HostCmd_DS_COMMAND * resp)
{
                          ...
                          HostCmd_DS_COMMAND * cmd, void *InfoBuf)
{
    ...
    cmd->Size = wlan_cpu_to_le16(sizeof(cmd->params.queueConfig.action)
                                 +
                                 sizeof(cmd->params.queueConfig.accessCategory)
                                 +
                              ...
                              const HostCmd_DS_COMMAND * resp)
{
    ...
    memcpy(pQConfigCmd->tlvBuffer, pCmdResp->tlvBuffer, pQConfigCmd->tlvBufLen);
...
wlan_wmm_queue_config_ioctl(wlan_private * priv, struct iwreq *wrq)
{
    ...
    if (copy_from_user(&queueConfigIoctl,
        ...
        queueConfigCmd.msduLifetimeExpiry = queueConfigIoctl.msduLifetimeExpiry;
...
wlan_cmdresp_wmm_queue_stats(wlan_private * priv,
                             const HostCmd_DS_COMMAND * resp)
{
    HostCmd_DS_WMM_QUEUE_STATS *pQueueStats = NULL;
...
wlan_wmm_queue_stats_ioctl(wlan_private * priv, struct iwreq *wrq)
{
    ...
    if (copy_from_user(&queueStatsIoctl,
                       wrq->u.data.pointer,
                       MIN(wrq->u.data.length, sizeof(queueStatsIoctl))) != 0) {
        /* copy_from_user failed */
 ...
 */
int
wlan_cmdresp_wmm_ts_status(wlan_private * priv, const HostCmd_DS_COMMAND * resp)
{
    HostCmd_DS_WMM_TS_STATUS *pTsStatus = NULL;
    ...
    pTsStatus = (HostCmd_DS_WMM_TS_STATUS *) priv->adapter->CurCmd->pdata_buf;
