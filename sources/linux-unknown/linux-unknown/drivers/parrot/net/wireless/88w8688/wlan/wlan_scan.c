...
/**
 * @brief Number of channels to scan per firmware scan command issuance when associated.
 *
 *  Number restricted to prevent hitting the limit on the amount of scan data
 *  returned in a single firmware scan command.
 */
#define MRVDRV_CHANNELS_PER_SCAN_CMD_CONNECTED    1
...
wlan_is_network_compatible(wlan_adapter * Adapter, int index, int mode)
{
    ...
    if (pBSSDesc->InfrastructureMode == mode) {
        if (Adapter->SecInfo.WEPStatus == Wlan802_11WEPDisabled
            ...
            return index;
                   ...
                   && ((pBSSDesc->pWpaIE) &&
                       ((*(pBSSDesc->pWpaIE)).VendHdr.ElementId == WPA_IE))
                   && !Adapter->AdhocAESEnabled
                   /* Privacy bit may NOT be set in some APs like LinkSys
                      WRT54G && pBSSDesc->Privacy */
            ) {
            /* WPA enabled */
            PRINTM(INFO,
        ...
        PRINTM(INFO, "wlan_is_network_compatible() FAILED: index=%d wpa_ie=%#x "
               "wpa2_ie=%#x WEP=%s WPA=%s WPA2=%s EncMode=%#x privacy=%#x\n",
               index,
               (pBSSDesc->pWpaIE) ? (*(pBSSDesc->pWpaIE)).VendHdr.ElementId : 0,
               (pBSSDesc->pRsnIE) ? (*(pBSSDesc->pRsnIE)).IeeeHdr.ElementId : 0,
...
wlan_scan_setup_scan_config(wlan_private * priv,
                            ...
                            int *pMaxChanPerScan,
                            BOOLEAN * pFilteredScan, BOOLEAN * pScanCurrentOnly)
{
    wlan_adapter *Adapter = priv->adapter;
    ...
    /* The tlvBufferLen is calculated for each scan command.  The TLVs added in 
       this routine will be preserved since the routine that sends the command
       will append channelTLVs at *ppChanTlvOut.  The difference between the
       *ppChanTlvOut and the tlvBuffer start will be used to calculate the size 
       of anything we add in this routine. */
    pScanCfgOut->tlvBufferLen = 0;

    /* Running tlv pointer.  Assigned to ppChanTlvOut at end of function so
    ...
    *pScanCurrentOnly = FALSE;
    ...
    if (pUserScanIn) {
        ...
        memcpy(pScanCfgOut->specificBSSID,
               pUserScanIn->specificBSSID, sizeof(pScanCfgOut->specificBSSID));
    ...
    if (*pFilteredScan)
        *pMaxChanPerScan = MRVDRV_MAX_CHANNELS_PER_SPECIFIC_SCAN;
        ...
        if (Adapter->MediaConnectStatus == WlanMediaStateConnected)
            *pMaxChanPerScan = MRVDRV_CHANNELS_PER_SCAN_CMD_CONNECTED;
        else
    ...
    if (numProbes) {
        ...
        pNumProbesTlv->Header.Len = wlan_cpu_to_le16(pNumProbesTlv->Header.Len);
    }

    /* 
     * Set the output for the channel TLV to the address in the tlv buffer
     *   past any TLVs that were added in this function (SSID, numProbes).
                       ...
                       ChanScanParamSet_t * pScanChanList)
{
    ...
    /* Loop through the desired channel list, sending a new firmware scan
       commands for each maxChanPerScan channels (or for 1,6,11 individually if 
       configured accordingly) */
    while (pTmpChan->ChanNumber) {
#define MAX_REKEY_TIME		(HZ)
        if (priv->adapter->RekeyFlag) {
            if ((jiffies - priv->adapter->RekeyAge) > MAX_REKEY_TIME)
                priv->adapter->RekeyFlag = FALSE;
            else {
                PRINTM(CMND, "Rekey in process...\n");
                break;
            }
        }
        tlvIdx = 0;
        ...
        while (tlvIdx < maxChanPerScan && pTmpChan->ChanNumber && !doneEarly) {
            ...
            PRINTM(INFO, "Scan: Chan(%3d), Radio(%d), Mode(%d,%d), Dur(%d)\n",
                   ...
                   wlan_le16_to_cpu(pTmpChan->MaxScanTime));
            ...
            totalscantime += wlan_le16_to_cpu(pTmpChan->MaxScanTime);
...
wlan_scan_create_brief_table_entry(u8 ** ppBuffer, BSSDescriptor_t * pBSSDesc)
{
    ...
    if (copy_to_user(pTmpBuf, pBSSDesc->TimeStamp, sizeof(pBSSDesc->TimeStamp))) {
    ...
    if (pBSSDesc->pWpaIE) {
        if ((*(pBSSDesc->pWpaIE)).VendHdr.ElementId == WPA_IE) {
            ieLen =
                sizeof(IEEEtypes_Header_t) + (*(pBSSDesc->pWpaIE)).VendHdr.Len;
    ...
    if (pBSSDesc->pRsnIE) {
        if ((*(pBSSDesc->pRsnIE)).IeeeHdr.ElementId == RSN_IE) {
            ieLen =
                sizeof(IEEEtypes_Header_t) + (*(pBSSDesc->pRsnIE)).IeeeHdr.Len;
 ...
 *  @param ppTsfTlv    Output parameter: Pointer to the request TLV if found
 *
 *  @return            void
 */
static void
    ...
    while (tlvBufLeft >= sizeof(MrvlIEtypesHeader_t)) {
        ...
        if (reqTlvType == tlvType) {
            ...
            case TLV_TYPE_CHANNELBANDLIST:
                PRINTM(INFO, "SCAN_RESP: CHANNEL BAND LIST TLV, len = %d\n",
                       tlvLen);
                *ppTlv = (MrvlIEtypes_Data_t *) pCurrentTlv;
                break;
            default:
        ...
        if (*ppTlv) {
            HEXDUMP("SCAN_RESP: TLV Buf", (u8 *) * ppTlv, tlvLen);
            break;
        }
    ...
    while (bytesLeftForCurrentBeacon >= 2) {
        ...
        case VENDOR_SPECIFIC_221:
            ...
            if (!memcmp(pVendorIe->VendHdr.Oui, wpa_oui, sizeof(wpa_oui))) {
                pBSSEntry->pWpaIE = (IEEEtypes_VendorSpecific_t *) pCurrentPtr;
                       ...
                       u8 * bssid, int mode)
{
    ...
    /* Loop through the table until the maximum is reached or until a match is
       found based on the bssid field comparison */
    for (i = 0;
...
wlan_scan_delete_table_entry(wlan_private * priv, int tableIdx)
{
    ...
    if (beaconBufAdj) {
        ...
        /* Remove the entry's buffer space, decrement the table end pointer by
        ...
        PRINTM(INFO,
               ...
               tableIdx,
               pBeaconBuf,
               pBeaconBuf + beaconBufAdj, Adapter->pBeaconBufEnd - pBeaconBuf);
    ...
    /* Shift all of the entries after the tableIdx back by one, compacting the
    ...
    for (delIdx = tableIdx; (delIdx + 1) < Adapter->NumInScanTable; delIdx++) {
        ...
        /* Adjust this entry's pointer to its beacon buffer based on the
           removed/compacted entry from the deleted index.  Don't decrement if
           the buffer pointer is NULL (no data stored for this entry). */
              ...
              struct iw_param *vwrq, char *extra)
{
    ...
    if (Adapter->MediaConnectStatus == WlanMediaStateConnected) {
        if (Adapter->SecInfo.WPAEnabled || Adapter->SecInfo.WPA2Enabled) {
            if (!Adapter->IsGTK_SET) {
                PRINTM(MSG, "Scan blocked during WPA/WAPI handshaking\n");
    ...
    if ((dwrq->flags & IW_SCAN_THIS_ESSID) &&
        ...
        if (req->essid_len <= MRVDRV_MAX_SSID_LENGTH) {
            ...
            memcpy(scanCfg.ssidList[0].ssid, (u8 *) req->essid, req->essid_len);
              ...
              struct iw_point *dwrq, char *extra)
{
    ...
    for (i = 0; i < Adapter->NumInScanTable; i++) {
        ...
        if (Adapter->BcnNFlast == 0) {
            iwe.u.qual.noise = MRVDRV_NF_DEFAULT_SCAN_VALUE;
        } else {
            iwe.u.qual.noise = Adapter->BcnNFlast;
        }
        if ((Adapter->InfrastructureMode == Wlan802_11IBSS) &&
            ...
            ret = wlan_prepare_cmd(priv, HostCmd_CMD_RSSI_INFO,
                                   HostCmd_ACT_GEN_GET,
            ...
            iwe.u.qual.level = Adapter->DataRSSIAvg;
        }
        iwe.u.qual.qual =
            wlan_rssi_to_quality((s16) (iwe.u.qual.level - 0x100));
        ...
        if (Adapter->ScanTable[i].pWpsIE) {
            if ((*(Adapter->ScanTable[i].pWpsIE)).VendHdr.ElementId == WPS_IE) {
                ...
                ptr = buf;
                memcpy(buf, pRawData,
                       (*(Adapter->ScanTable[i].pWpsIE)).VendHdr.Len + 2);
                iwe.cmd = IWEVGENIE;
                iwe.u.data.length =
                    (*(Adapter->ScanTable[i].pWpsIE)).VendHdr.Len + 2;
                iwe.len = IW_EV_POINT_LEN + iwe.u.data.length;
...
wlan_get_scan_table_ioctl(wlan_private * priv, struct iwreq *wrq)
{
    ...
    if (copy_from_user(&scanStart, wrq->u.data.pointer, sizeof(scanStart)) != 0) {
    ...
    if (scanStart == 0) {
        ...
        while (spaceLeft && (scanStart + numScansDone < Adapter->NumInScanTable)
               && (retcode == WLAN_STATUS_SUCCESS)) {
...
wlan_set_user_scan_ioctl(wlan_private * priv, struct iwreq *wrq)
{
    ...
    if (OS_ACQ_SEMAPHORE_BLOCK(&priv->adapter->ReassocSem)) {
        PRINTM(ERROR, "Acquire semaphore error, wlan_set_user_scan_ioctl\n");
        LEAVE();
        return -EBUSY;
    }
#endif
...
wlan_ret_802_11_scan_store_beacon(wlan_private * priv,
                                  int beaconIdx,
                                  int numInTable, BSSDescriptor_t * pNewBeacon)
{
    wlan_adapter *Adapter = priv->adapter;
    u8 *pBcnStore;
    int newBcnSize;
    ...
    if (Adapter->ScanTable[beaconIdx].pBeaconBuf != NULL) {
        ...
        if (newBcnSize == oldBcnSize) {
             ...
             */
            if (oldBcnSize < bcnSpace && newBcnSize <= oldBcnSize) {
                /* 
            ...
            pNewBeacon->wpaOffset = Adapter->ScanTable[beaconIdx].wpaOffset;
            pNewBeacon->rsnOffset = Adapter->ScanTable[beaconIdx].rsnOffset;
            pNewBeacon->wpsOffset = Adapter->ScanTable[beaconIdx].wpsOffset;
        }
...
wlan_ret_802_11_scan(wlan_private * priv, HostCmd_DS_COMMAND * resp)
{
    ...
    int tlvBufSize;
    u64 tsfVal;
    MrvlIEtypes_ChanBandListParamSet_t *pChanBandTlv;
    ChanBandParamSet_t *pChanBand;
    u8 band;
    BOOLEAN bgScanResp;
    ...
    wlan_ret_802_11_scan_get_tlv_ptrs(pTlv, tlvBufSize, TLV_TYPE_TSFTIMESTAMP,
                                      (MrvlIEtypes_Data_t **) & pTsfTlv);

    /* Search the TLV buffer space in the scan response for any valid TLVs */
    wlan_ret_802_11_scan_get_tlv_ptrs(pTlv, tlvBufSize,
                                      TLV_TYPE_CHANNELBANDLIST,
                                      (MrvlIEtypes_Data_t **) & pChanBandTlv);
    /* 
    ...
    for (idx = 0; idx < pScan->NumberOfSets && bytesLeft; idx++) {
        ...
        if ((wlan_interpret_bss_desc_with_ie(&newBssEntry,
            ...
            band = BAND_G;
            if (pChanBandTlv) {
                pChanBand = &pChanBandTlv->ChanBandParam[idx];
                band =
                    radio_type_to_band(pChanBand->
                                       RadioType & (BIT(0) | BIT(1)));
            }
...
wlan_do_bgscfg_ioctl(wlan_private * priv, struct ifreq *req)
{
    ...
    Adapter->bgScanConfig->Enable = pstBgScanCfg->Enable;

    ret = wlan_prepare_cmd(priv,
                           HostCmd_CMD_802_11_BG_SCAN_CONFIG,
                           action, HostCmd_OPTION_WAITFORRSP, 0, pstBgScanCfg);

    if (!ret && action == HostCmd_ACT_GEN_GET) {
        if (copy_to_user(req->ifr_data + SKIP_CMDNUM + SKIP_SIZE, pstBgScanCfg,
                               ...
                               int cmd_action, void *pdata_buf)
{
    ...
    cmd->Size = wlan_cpu_to_le16((priv->adapter->bgScanConfigSize) + S_DS_GEN);
