...
wlan_proc_entry(wlan_private * priv, struct net_device *dev)
{
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,26)
    /* Check if mwlan directory already exists */
    for (r = r->subdir; r; r = r->next) {
        if (r->namelen && !strcmp("mwlan", r->name)) {
            /* Directory exists */
    ...
    if (!priv->proc_mwlan) {
        ...
        if (!priv->proc_mwlan)
            ...
            atomic_set(&priv->proc_mwlan->count, 1);
    } else {
        atomic_inc(&priv->proc_mwlan->count);
    }
    priv->proc_entry = proc_mkdir(dev->name, priv->proc_mwlan);
    strcpy(priv->proc_entry_name, dev->name);
    if (priv->proc_entry) {
        r = create_proc_read_entry("info", 0, priv->proc_entry, wlan_proc_read,
                                   dev);
    }
}
...
wlan_proc_remove(wlan_private * priv)
{
    if (priv->proc_mwlan) {
        ...
        atomic_dec(&priv->proc_mwlan->count);
        if (atomic_read(&(priv->proc_mwlan->count)) == 0)
