...
/**
 * the placeholder for ABG cfp table when band set to "abg"
 */
static CHANNEL_FREQ_POWER cfp_ABG[MAX_CFP_LIST_NUM];
...
wlan_get_region_cfp_table(u8 region, u8 band, int *cfp_no)
{
    ...
    for (i = 0; i < sizeof(region_cfp_table) / sizeof(region_cfp_table_t); i++) {
        ...
        if (region_cfp_table[i].region == region) {
            if (band == (ALL_802_11_BANDS)) {
                memcpy(cfp_ABG, region_cfp_table[i].cfp_BG,
                       region_cfp_table[i].cfp_no_BG *
                       sizeof(CHANNEL_FREQ_POWER));
                memcpy(cfp_ABG + region_cfp_table[i].cfp_no_BG,
                       region_cfp_table[i].cfp_A,
                       region_cfp_table[i].cfp_no_A *
                       sizeof(CHANNEL_FREQ_POWER));
                *cfp_no =
                    region_cfp_table[i].cfp_no_BG +
                    region_cfp_table[i].cfp_no_A;
...
wlan_interrupt(wlan_private * priv)
{
    ...
    /* sd_set_busclock((psd_device)priv->wlan_dev.card, 0); */
