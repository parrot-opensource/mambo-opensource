 ...
 */ 
    static int
get_common_rates(wlan_adapter * Adapter, u8 * rate1,  int rate1_size,
 ...
 */ 
    static int
setup_rates_from_bssdesc(wlan_adapter * Adapter,  BSSDescriptor_t * pBSSDesc,
                          u8 * pOutRates,  int *pOutRatesSize) 
{
     ...
     case BAND_A | BAND_B | BAND_G:
      ...
      default:
        ...
        if (get_common_rates
            (Adapter, pOutRates, WLAN_SUPPORTED_RATES,  card_rates,
             card_rates_size)) {
         *pOutRatesSize = 0;
...
wlan_get_assoc_rsp_ioctl(wlan_private * priv, struct iwreq *wrq) 
{
     ...
     int copySize;
      ENTER();
        ...
        /*   Set the amount to copy back to the application as the minimum of
           the  available assoc resp data or the buffer provided by the
           application  */ 
                ...
                struct iw_point *dwrq, char *extra) 
{
     ...
     int i;
      ...
      if (Adapter->InfrastructureMode == Wlan802_11Infrastructure) {
        ...
        /*   The MediaConnectStatus change can be removed later when  the ret
...
wlan_associate(wlan_private * priv, BSSDescriptor_t * pBSSDesc) 
{
     ...
     int reassocAttempt = FALSE;
      ...
      memcpy(&currentBSSID,  &Adapter->CurBssParams.BSSDescriptor.MacAddress,
              sizeof(currentBSSID));
      if (Adapter->MediaConnectStatus == WlanMediaStateConnected) {
         reassocAttempt = TRUE;
        ...
        Adapter->assocRspSize = 0;
      ret =
        wlan_prepare_cmd(priv, HostCmd_CMD_802_11_ASSOCIATE,  0,
                         HostCmd_OPTION_WAITFORRSP,  0, pBSSDesc);
...
#define ERROR_STA_NOT_SUPPORT_SPECIFIC_AUTH 13
        if ((Adapter->MediaConnectStatus != WlanMediaStateConnected) && 
            ...
            Adapter->assocRspSize &&
             (pAssocRsp->StatusCode == ERROR_STA_NOT_SUPPORT_SPECIFIC_AUTH)) {
         if (Adapter->SecInfo.AuthenticationMode == Wlan802_11AuthModeOpen)
...
wlan_associate_to_table_idx(wlan_private * priv, int tableIdx) 
{
     ...
     int ret;
      ENTER();
        ...
        /*   Check index in table, subtract 1 if within range and call
           association  sub-function.  ScanTable[] is 0 based, parameter is 1
           based to  conform with IW_ENCODE_INDEX flag parameter passing in
           iwconfig/iwlist  */ 
...
wlan_start_adhoc(wlan_private * priv, WLAN_802_11_SSID * AdhocSSID) 
{
     ...
     PRINTM(INFO, "CurBssParams.band = %d\n", priv->adapter->CurBssParams.band);
      ret =
        wlan_prepare_cmd(priv, HostCmd_CMD_802_11_AD_HOC_START,  0,
                         HostCmd_OPTION_WAITFORRSP,  0, AdhocSSID);
      LEAVE();
     return ret;
 }
...
wlan_join_adhoc(wlan_private * priv, BSSDescriptor_t * pBSSDesc) 
{
     ...
     PRINTM(INFO, "wlan_join_adhoc: ssid len =%u\n",
        ...
        if (Adapter->CurBssParams.BSSDescriptor.Ssid.SsidLength 
            &&!wlan_ssid_cmp(&pBSSDesc->Ssid,
                             ...
                             Ssid)   &&(Adapter->CurBssParams.BSSDescriptor.
                                        InfrastructureMode ==
                                         Wlan802_11IBSS)) {
          PRINTM(INFO,
     ...
     PRINTM(INFO, "CurBssParams.band = %c\n", Adapter->CurBssParams.band);
      ret =
        wlan_prepare_cmd(priv, HostCmd_CMD_802_11_AD_HOC_JOIN,  0,
                         HostCmd_OPTION_WAITFORRSP,  0, pBSSDesc);
      LEAVE();
     return ret;
 }
          ...
          if (Adapter->InfrastructureMode == Wlan802_11Infrastructure) {
             memcpy((void *) saddr.sa_data, 
                    (void *) &Adapter->CurBssParams.BSSDescriptor.MacAddress,
                     ETH_ALEN);
             ret =
                wlan_prepare_cmd(priv, HostCmd_CMD_802_11_DEAUTHENTICATE,  0,
                                 HostCmd_OPTION_WAITFORRSP |
                                 HostCmd_OPTION_TIMEOUT,  0, (void *) &saddr);
          } else if (Adapter->InfrastructureMode == Wlan802_11IBSS) {
             ret =
                wlan_prepare_cmd(priv, HostCmd_CMD_802_11_AD_HOC_STOP,  0,
                                 HostCmd_OPTION_WAITFORRSP |
         ...
         if (Adapter->InfrastructureMode == Wlan802_11Infrastructure) {
             if (memcmp(Adapter->PreviousBSSID, zeroMac,  sizeof(zeroMac)) != 0) {
                 ...
                 PRINTM(INFO,
                  ...
                  i =
                    wlan_find_ssid_in_list(Adapter,  &Adapter->PreviousSSID,
...
wlan_cmd_append_generic_ie(wlan_private * priv, u8 ** ppBuffer) 
{
     ...
     if (*ppBuffer == 0)
        ...
        if (Adapter->genIeBufferLen) {
         PRINTM(CMND, "append generic %d to %p\n", Adapter->genIeBufferLen,
                 *ppBuffer);
     ...
     if (*ppBuffer == 0)
        ...
        if (Adapter->mrvlAssocTlvBufferLen) {
         PRINTM(INFO, "append tlv %d to %p\n",  Adapter->mrvlAssocTlvBufferLen,
            ...
            memcpy(*ppBuffer,  Adapter->mrvlAssocTlvBuffer,
                    Adapter->mrvlAssocTlvBufferLen);
 ...
 */ 
    int
wlan_cmd_802_11_associate(wlan_private * priv,  HostCmd_DS_COMMAND * cmd,
                           void *pdata_buf) 
{
     wlan_adapter * Adapter = priv->adapter;
     ...
     pos = (u8 *) pAsso;
      ...
      memcpy(pAsso->PeerStaAddr,  pBSSDesc->MacAddress,
     ...
     wlan_cmd_append_generic_ie(priv, &pos);
      ...
      if (wlan_create_dnld_countryinfo_11d(priv, pBSSDesc->bss_band)) {
     ...
     if (wlan_parse_dnld_countryinfo_11d(priv)) {
        ...
        /*   Call 11h join API after capability bits are set so adhoc/infra
           11h  behavior can be properly triggered.  pos modified if data is
           appended  */ 
 ...
 */ 
    int
wlan_cmd_802_11_ad_hoc_start(wlan_private * priv,  HostCmd_DS_COMMAND * cmd,
                              void *pssid) 
{
     wlan_adapter * Adapter = priv->adapter;
     ...
     Adapter->pAttemptedBSSDesc = pBSSDesc;
        ...
        /*   Fill in the parameters for 2 data structures:  1.
           HostCmd_DS_802_11_AD_HOC_START Command  2. pBSSDesc  Driver will
           fill up SSID, BSSType,IBSS param, Physical Param,  probe delay, and
           Cap info.  Firmware will fill up beacon period, Basic rates  and
           operational rates.  */ 
...
#define DS_PARA_IE_LEN  1
     ...
     adhs->PhyParamSet.DsParamSet.CurrentChan = Adapter->AdhocChannel;
      memcpy(&pBSSDesc->PhyParamSet,  &adhs->PhyParamSet,
...
#define IBSS_PARA_IE_LEN  2
     ...
     memcpy(&pBSSDesc->SsParamSet,  &adhs->SsParamSet,
...
#define AD_HOC_CAP_PRIVACY_ON 1
      ...
      PRINTM(INFO, "ADHOC_S_CMD: Rates=%02x %02x %02x %02x \n",
              adhs->DataRate[0], adhs->DataRate[1],  adhs->DataRate[2],
             adhs->DataRate[3]);
      ...
      if (wlan_create_dnld_countryinfo_11d(priv, Adapter->CurBssParams.band)) {
        ...
        /*   Call 11h start API to add any 11h flags/elements as TLV
           parameters  */ 
        cmdAppendSize +=
        wlan_11h_process_start(priv, &pos, &adhs->Cap,  Adapter->AdhocChannel,
                                &pBSSDesc->wlan_11h_bss_info);
       if (Adapter->SecInfo.WPAEnabled  ) {
         ...
         if (pRsnTlv->Header.Len <= (sizeof(Adapter->Wpa_ie) - 2))
          ...
          HEXDUMP("ADHOC_S_CMD: RSN IE", (u8 *) pRsnTlv,
                   sizeof(pRsnTlv->Header) + pRsnTlv->Header.Len);
         pos += sizeof(pRsnTlv->Header) + pRsnTlv->Header.Len;
 ...
 */ 
    int
wlan_cmd_802_11_ad_hoc_join(wlan_private * priv,  HostCmd_DS_COMMAND * cmd,
                             void *pdata_buf) 
{
     wlan_adapter * Adapter = priv->adapter;
...
#define USE_G_PROTECTION	0x02    
     ...
     Adapter->pAttemptedBSSDesc = pBSSDesc;
      ...
      memcpy(&pAdHocJoin->BssDescriptor.BSSID,  &pBSSDesc->MacAddress,
              ETH_ALEN);
      memcpy(&pAdHocJoin->BssDescriptor.SSID,  &pBSSDesc->Ssid.Ssid,
              pBSSDesc->Ssid.SsidLength);
      memcpy(&pAdHocJoin->BssDescriptor.PhyParamSet,  &pBSSDesc->PhyParamSet,
              sizeof(IEEEtypes_PhyParamSet_t));
      memcpy(&pAdHocJoin->BssDescriptor.SsParamSet,  &pBSSDesc->SsParamSet,
              sizeof(IEEEtypes_SsParamSet_t));
     ...
     Adapter->CurBssParams.band = pBSSDesc->bss_band;
      ...
      if (wlan_parse_dnld_countryinfo_11d(priv)) {
        ...
        /*   Call 11h join API after capability bits are set so  adhoc/infra
           ...
           appended  */ 
        cmdAppendSize +=
        wlan_11h_process_join(priv, &pos,  &pAdHocJoin->BssDescriptor.Cap,
                               pBSSDesc->Channel,
                               &pBSSDesc->wlan_11h_bss_info);
      if (Adapter->SecInfo.WPAEnabled) {
         pRsnTlv = (MrvlIEtypes_RsnParamSet_t *) pos;
...
wlan_ret_802_11_associate(wlan_private * priv,  HostCmd_DS_COMMAND * resp) 
{
     ...
     int ratesSize;
      ...
      PRINTM(INFO, "ASSOC_RESP: %s\n", pBSSDesc->Ssid.Ssid);
        ...
        memcpy(&Adapter->CurBssParams.BSSDescriptor,  pBSSDesc,
     ...
     BSSDescriptor_t * pBSSDesc;
      ...
      pBSSDesc = Adapter->pAttemptedBSSDesc;
        ...
        if (Result) {
         ...
         if (Adapter->MediaConnectStatus == WlanMediaStateConnected)
             wlan_reset_connect_state(priv);
          memset(&Adapter->CurBssParams.BSSDescriptor,  0x00,
      ...
      if (Command == HostCmd_CMD_802_11_AD_HOC_START) {
         PRINTM(INFO, "ADHOC_S_RESP  %s\n", pBSSDesc->Ssid.Ssid);
          ...
          Adapter->AdhocState = ADHOC_STARTED;
            ...
            /*   Now the join cmd should be successful  If BSSID has changed
            ...
            /* Make a copy of current BSSID descriptor, only needed for join
               since  the current descriptor is already being used for adhoc
               start */ 
            memcpy(&Adapter->CurBssParams.BSSDescriptor,  pBSSDesc,
                    sizeof(BSSDescriptor_t));
        ...
        current->flags |= PF_NOFREEZE;
         ...
         if (Adapter->SurpriseRemoved) {
          ...
          if (i < 0) {
                ...
                i =
                wlan_find_ssid_in_list(Adapter,  &Adapter->PreviousSSID,  NULL,
                                        Adapter->InfrastructureMode);
         }
          if (i >= 0) {
             if (Adapter->SecInfo.WEPStatus == Wlan802_11WEPEnabled) {
                 ret =
                    wlan_prepare_cmd(priv,  HostCmd_CMD_802_11_SET_WEP,
                                      HostCmd_ACT_ADD,
          ...
          OS_REL_SEMAPHORE(&Adapter->ReassocSem);
        settimer: if (Adapter->MediaConnectStatus ==
