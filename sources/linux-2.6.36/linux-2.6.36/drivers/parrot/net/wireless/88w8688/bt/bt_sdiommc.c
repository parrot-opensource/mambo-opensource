...
sd_read_cmd52_val(bt_private * priv)
{
    ...
    val = sdio_readb(card->func, CARD_RX_UNIT_REG, &ret);
...
sd_interrupt(struct sdio_func *func)
{
    ...
    if (card && card->priv) {
        ...
        if (sbi_get_int_status(priv, &ireg)) {
            PRINTM(ERROR, "%s: reading HOST_INTSTATUS_REG failed\n",
                   __FUNCTION__);
        } else
            PRINTM(INFO, "%s: HOST_INTSTATUS_REG %#x\n", __FUNCTION__, ireg);
...
sbi_get_int_status(bt_private * priv, u8 * ireg)
{
    ...
    *ireg = 0;
    ...
    if (ret) {
        PRINTM(WARN, "sdio_readb: read int status register failed\n"); //XXX check bt
        ret = BT_STATUS_FAILURE;
        goto done;
    }
    if (sdio_ireg != 0) {
