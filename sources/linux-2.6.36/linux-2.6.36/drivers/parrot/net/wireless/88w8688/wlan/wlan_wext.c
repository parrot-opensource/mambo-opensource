                       ...
                       struct iw_point *dwrq, char *extra)
{
    ...
    if (dwrq->flags & (IW_ENCODE_RESTRICTED | IW_ENCODE_OPEN)) {
        ...
        case IW_ENCODE_RESTRICTED:
            ...
            Adapter->AuthType = AUTHTYPE_SHARED;
            PRINTM(CMND, "Auth mode restricted!\n");
            break;
        case IW_ENCODE_OPEN:
            /* iwconfig ethX key [2] open */
            Adapter->SecInfo.AuthenticationMode = Wlan802_11AuthModeOpen;
            Adapter->AuthType = AUTHTYPE_OPEN;
            PRINTM(CMND, "Auth mode open!\n");
            break;
        case IW_ENCODE_RESTRICTED | IW_ENCODE_OPEN:
            /* iwconfig ethX key [2] open restricted */
            PRINTM(CMND, "Auth mode auto!\n");
                ...
                struct iw_request_info *info, struct iw_point *dwrq, u8 * extra)
{
    ...
    if ((adapter->SecInfo.WEPStatus == Wlan802_11WEPEnabled)
        ...
        || adapter->SecInfo.WPA2Enabled || adapter->AdhocAESEnabled) {
                ...
                struct iw_point *dwrq, char *extra)
{
    ...
    ENTER();
    ret =
        wlan_get_gen_ie_helper(netdev_priv(dev), dwrq->pointer, &dwrq->length);
    LEAVE();
    return ret;
}
...
wlan_setauthalg(wlan_private * priv, int alg)
{
    ...
    case IW_AUTH_ALG_SHARED_KEY:
        PRINTM(CMND, "Auth mode shared key!\n");
        Adapter->SecInfo.AuthenticationMode = Wlan802_11AuthModeShared;
        Adapter->AuthType = AUTHTYPE_SHARED;
        break;
    case IW_AUTH_ALG_LEAP:
        /* clear WPA IE */
        PRINTM(CMND, "Auth mode LEAP!\n");
 ...
 */
static int
wlan_set_priv(struct net_device *dev, struct iw_request_info *info,
              struct iw_point *dwrq, char *extra)
{
    int ret = 0;
    wlan_private *priv = (wlan_private *) netdev_priv(dev);
    ...
    PRINTM(CMND, "SIOCSIWPRIV requst = %s\n", buf);
    if (strncmp(buf, "RSSI", strlen("RSSI")) == 0) {
        ...
        PRINTM(CMND, "tx rate=%d\n", (int) priv->adapter->TxRate);
        len =
            sprintf(buf, "LinkSpeed %d\n",
                    (int) (index_to_data_rate(priv->adapter->TxRate) * 500000 /
                           1000000)) + 1;
    } else if (strncmp(buf, "MACADDR", strlen("MACADDR")) == 0) {
        len =
            sprintf(buf, "Macaddr = %02X:%02X:%02X:%02X:%02X:%02X\n",
                    priv->adapter->CurrentAddr[0],
                    priv->adapter->CurrentAddr[1],
                    priv->adapter->CurrentAddr[2],
                    priv->adapter->CurrentAddr[3],
                    priv->adapter->CurrentAddr[4],
                    priv->adapter->CurrentAddr[5]) + 1;
        ...
        priv->adapter->ScanType = HostCmd_SCAN_TYPE_ACTIVE;
        PRINTM(CMND, "Set Active Scan\n");
        len = sprintf(buf, "OK\n") + 1;
        ...
        priv->adapter->ScanType = HostCmd_SCAN_TYPE_PASSIVE;
        PRINTM(CMND, "Set Passive Scan\n");
        len = sprintf(buf, "OK\n") + 1;
    } else if (strncmp(buf, "POWERMODE", strlen("POWERMODE")) == 0) {
        powermode = buf + strlen("POWERMODE") + 1;
        if (*powermode == '1') {
            PRINTM(CMND, "Disable power save mode\n");
            priv->adapter->PSMode = Wlan802_11PowerModeCAM;
            if (priv->adapter->PSState != PS_STATE_FULL_POWER) {
                wlan_exit_ps(priv, HostCmd_OPTION_WAITFORRSP);
            }
        } else if (*powermode == '0') {
            PRINTM(CMND, "Enable power save mode\n");
            if (priv->adapter->PSMode == Wlan802_11PowerModeCAM) {
                priv->adapter->PSMode = Wlan802_11PowerModeMAX_PSP;
            ...
            PRINTM(ERROR, "unsupported power mode\n");
            ret = -EFAULT;
            goto done;
        }
        len = sprintf(buf, "OK\n") + 1;
        ...
        len = sprintf(buf, "OK\n") + 1;
        PRINTM(WARN, "Unknow PRIVATE command: %s, ignored\n", buf);
    }
    PRINTM(CMND, "PRIV Command return: %s, length=%d\n", buf, len);
    dwrq->length = len;
        ...
        if ((pVendorIe->ElementId == RSN_IE)
            ...
            if (ie_len < (sizeof(Adapter->genIeBuffer)
                ...
                /* Test to see if it is a WPS IE, if so, enable wps session
                   flag */
                pVendorIe = (IEEEtypes_VendorHeader_t *) ie_data_ptr;
                if ((pVendorIe->ElementId == WPS_IE)
                    && (!memcmp(pVendorIe->Oui, wps_oui, sizeof(wps_oui)))) {
 ...
 */
u8
wlan_rssi_to_quality(s16 rssi)
{
/** Macro for RSSI range */
#define RSSI_NO_SIGNAL -90
#define RSSI_VERY_LOW  -80
#define RSSI_LOW       -70
#define RSSI_GOOD      -60
#define RSSI_VERY_GOOD -50
#define RSSI_INVALID   0
    if (rssi <= RSSI_NO_SIGNAL || rssi == RSSI_INVALID)
        return 0;
    else if (rssi <= RSSI_VERY_LOW)
        return 1;
    else if (rssi <= RSSI_LOW)
        return 2;
    else if (rssi <= RSSI_GOOD)
        return 3;
    else if (rssi <= RSSI_VERY_GOOD)
        return 4;
    else
        return 5;
}
...
wlan_get_wireless_stats(struct net_device *dev)
{
    ...
    if (Adapter->MediaConnectStatus == WlanMediaStateConnected) {
        ret = wlan_prepare_cmd(priv,
                               HostCmd_CMD_RSSI_INFO, HostCmd_ACT_GEN_GET,
                               wait_option, 0, NULL);
    }
    ...
    priv->wstats.qual.qual = wlan_rssi_to_quality((s16)
