...
wlan_process_tx(wlan_private * priv, struct sk_buff *skb)
{
    ...
    /* check headroom and tail room */
    if ((skb_headroom(skb) < (headlen + HEADER_ALIGNMENT)) ||
        (skb_tailroom(skb) < tailroom)) {
        PRINTM(DATA, "Tx: Insufficient skb headroom %d or tailroom %d\n",
               skb_headroom(skb), skb_tailroom(skb));
        /* Insufficient skb headroom - allocate a new skb */
        newskb =
            skb_copy_expand(skb, headlen + HEADER_ALIGNMENT, tailroom,
                            GFP_ATOMIC);
        if (!newskb) {
        ...
        kfree_skb(skb);
        skb = newskb;
        PRINTM(DATA, "new skb headroom %d, tailroom %d\n", skb_headroom(skb),
               skb_tailroom(skb));
    }
