...
#endif
    {.cmd = WLAN_SETNONE_GETNONE,.subcmd = WLANSOFTRESET,.fixsize = FALSE},
};
 ...
 */
static int
wlan_soft_reset(wlan_private * priv)
{
    wlan_adapter *Adapter = priv->adapter;
    ENTER();
    /* Disable interface */
    os_stop_queue(priv);
    netif_device_detach(priv->wlan_dev.netdev);

    if (Adapter->bHostSleepConfigured) {
        Adapter->HSCfg.conditions = HOST_SLEEP_CFG_CANCEL;
        wlan_prepare_cmd(priv, HostCmd_CMD_802_11_HOST_SLEEP_CFG, 0,
                         HostCmd_OPTION_WAITFORRSP, 0, &Adapter->HSCfg);
    }
#ifdef REASSOCIATION
    if (Adapter->ReassocTimerIsSet == TRUE) {
        wlan_cancel_timer(&Adapter->MrvDrvTimer);
    ...
    wlan_prepare_cmd(priv, HostCmd_CMD_SOFT_RESET,
...
static int
wlan_disable_auto_channel_switch(wlan_private * priv, struct iwreq *wrq)
{
    int data;
    int ret;
    int *val;
    ...
    ENTER();
    data = *((int *) (wrq->u.name + SUBCMD_OFFSET));
    PRINTM(INFO, "wlan_disable_auto_channel_switch: %d\n", data);
    switch (data) {
    case CMD_DISABLED:
    case CMD_ENABLED:
        ...
        ret = wlan_prepare_cmd(priv, HostCmd_CMD_802_11_SNMP_MIB,
                               ...
                               DisableAutoChannelSwitch_i, &data);

        Adapter->DisableChanSw = (u16) data;
    ...
    case CMD_GET:
        ret = wlan_prepare_cmd(priv,
                               ...
                               DisableAutoChannelSwitch_i, NULL);
        if (ret) {
            LEAVE();
            return ret;
        }
...
/** 
 *  @brief Get/Set auth type.
 *  @param priv         A pointer to wlan_private structure
 *  @param req			A pointer to ifreq structure
 *  @return 	   		WLAN_STATUS_SUCCESS --success, otherwise fail
 */
static int
wlan_auth_type(wlan_private * priv, struct iwreq *wrq)
{
    int data;
    wlan_adapter *Adapter = priv->adapter;
    int ret = 0;
    ...
    if ((int) wrq->u.data.length == 0) {
        ...
        PRINTM(CMND, "SET: auth_type %d\n", data);
        if ((data < AUTHTYPE_OPEN) || (data > AUTHTYPE_ALLOW_BOTH)) {
            ret = -EINVAL;
            goto done;
        }
        Adapter->AuthType = data;
    }
    wrq->u.data.length = 1;
  done:
    LEAVE();
...
wlan_drv_dbg(wlan_private * priv, struct iwreq *wrq)
{
    int data[4];
    int ret = WLAN_STATUS_SUCCESS;
    int data_len = wrq->u.data.length;
    ...
    if (wrq->u.data.length == 0) {
        ...
        if (copy_from_user(data, wrq->u.data.pointer, sizeof(int) * data_len)) {
            PRINTM(INFO, "Copy from user failed\n");
            ret = -EFAULT;
            goto drvdbgexit;
        }
...
wlan_hostcmd_ioctl(struct net_device *dev, struct ifreq *req, int cmd)
{
    ...
    memset(tempResponseBuffer, 0, MRVDRV_SIZE_OF_CMD_BUFFER); /* Parrot bug 96400 */
     ...
     */
    if (copy_from_user(pCmdPtr, wrq->u.data.pointer, data_len)) {
        PRINTM(INFO, "Copy from user failed\n");
...
wlan_get_snr(wlan_private * priv, struct iwreq *wrq)
{
    ...
    wlan_adapter *Adapter = priv->adapter;
    int data[4];
    int user_data_len = MIN(wrq->u.data.length, 4);
    ...
    if (wrq->u.data.length) {
        if (copy_from_user
            (data, wrq->u.data.pointer, user_data_len * sizeof(int))) {
            PRINTM(INFO, "Copy from user failed\n");
            LEAVE();
            return -EFAULT;
        }
...
wlan_regrdwr_ioctl(wlan_private * priv, struct ifreq *req)
{
    ...
    if (regrdwr.WhichReg == REG_MAC ||
        regrdwr.WhichReg == REG_BBP ||
        regrdwr.WhichReg == REG_RF || regrdwr.WhichReg == REG_PMIC) {
...
wlan_cmd53rdwr_ioctl(wlan_private * priv, struct ifreq *req)
{
    ...
    if (!(data = kmalloc(CMD53BUFLEN, GFP_KERNEL))) {
    ...
    total_len = (mode) ? blknum * blklen : blknum;
    ...
    if (rw == 0) {
        ...
        if (sdio_readsb(card->func, data, reg, total_len) < 0) {
            PRINTM(ERROR, "sdio_readsb: reading memory 0x%x failed\n", reg);
        }
        sdio_release_host(card->func);
    }

    if (copy_to_user(req->ifr_data, data, total_len)) {
        PRINTM(INFO, "Copy to user failed\n");
        kfree(data);
        LEAVE();
        return -EFAULT;
...
{
    wlan_adapter *Adapter = priv->adapter;
    int data[3] = { 0 }, data_length = wrq->u.data.length;
    int ret = WLAN_STATUS_SUCCESS;
    ...
    if (data_length >= 1 && data_length <= 3) {
        ...
        if (data[0] != HOST_SLEEP_CFG_CANCEL &&
            Adapter->bHostSleepConfigured == TRUE) {
            PRINTM(ERROR, "HS already configured\n");
            ret = -EFAULT;
            goto done;
        }
        if (data[0] == HOST_SLEEP_CFG_CANCEL &&
        ...
        PRINTM(INFO,
               ...
               Adapter->HSCfg.gap, Adapter->PSState, Adapter->HS_Activated);

        ret = wlan_prepare_cmd(priv, HostCmd_CMD_802_11_HOST_SLEEP_CFG, 0,
                               HostCmd_OPTION_WAITFORRSP, 0, &Adapter->HSCfg);
...
wlan_hssetpara_ioctl(wlan_private * priv, struct iwreq *wrq)
{
    ...
    if (Adapter->bHostSleepConfigured == TRUE) {
        PRINTM(ERROR, "HS already configured\n");
        ret = -EFAULT;
        goto done;
    }
 ...
 */
static int
wlan_custom_ie_ioctl(wlan_private * priv, struct ifreq *req)
{
    u16 action;
    wlan_ioctl_custom_ie custom_ie;
    int ret;

    ENTER();

    memset(&custom_ie, 0, sizeof(custom_ie));
    if (copy_from_user(&custom_ie, req->ifr_data, sizeof(wlan_ioctl_custom_ie))) {
    ...
    ret = wlan_prepare_cmd(priv,
                           HostCmd_CMD_MGMT_IE_LIST, action,
                           HostCmd_OPTION_WAITFORRSP, 0, &custom_ie);

    if (action == HostCmd_ACT_GEN_GET) {
        if (copy_to_user
            (req->ifr_data, &custom_ie, sizeof(wlan_ioctl_custom_ie))) {
 ...
 */
static int
wlan_read_eeprom(wlan_private * priv, struct iwreq *wrq)
{
    wlan_ioctl_regrdwr *regrdwr = NULL;
    u8 *pRdeeprom, buf[2];
    int length, ret = WLAN_STATUS_SUCCESS;
    ...
    if (copy_from_user(buf, wrq->u.data.pointer, sizeof(buf))) {
        PRINTM(INFO, "rdeeprom: Copy from user failed \n");
        LEAVE();
        return -EFAULT;
    }
    length = buf[1];
    pRdeeprom =
        (char *) kmalloc((length + sizeof(wlan_ioctl_regrdwr)), GFP_KERNEL);
    if (!pRdeeprom) {
        PRINTM(INFO, "rdeeprom: Allocate memory for EEPROM read failed\n");
        LEAVE();
        return -ENOMEM;
    }
    regrdwr = (wlan_ioctl_regrdwr *) pRdeeprom;
    regrdwr->Action = HostCmd_ACT_GEN_READ;
    regrdwr->WhichReg = REG_EEPROM;
    regrdwr->Offset = (u32) buf[0];
    regrdwr->NOB = (u16) buf[1];
    PRINTM(INFO, "Action: %d, Offset: %x, NOB: %02x\n", regrdwr->Action,
           regrdwr->Offset, regrdwr->NOB);

    ret = wlan_prepare_cmd(priv, HostCmd_CMD_802_11_EEPROM_ACCESS,
                           regrdwr->Action, HostCmd_OPTION_WAITFORRSP, 0,
                           pRdeeprom);

    if (!ret) {
        wrq->u.data.length = regrdwr->NOB;
        if (copy_to_user(wrq->u.data.pointer, &regrdwr->Value, regrdwr->NOB)) {
            PRINTM(INFO, "rdeeprom: Copy to user failed \n");
            ret = -EFAULT;
        }
    }
...
/** 
 *  @brief Set/get CTS power control
 *   
 *  @param priv		A pointer to wlan_private structure
 *  @param wreq		A pointer to iwreq structure
 *  @return    		WLAN_STATUS_SUCCESS --success, otherwise fail
 */
static int
wlan_cts_power_ioctl(wlan_private * priv, struct iwreq *wrq)
{
    int data = 0, ret = WLAN_STATUS_FAILURE;
    u16 action, powerctrl;
...
/** 
 *  @brief This function sets/gets current IEEE PS mode
 *   
 *  @param priv		A pointer to wlan_private structure
 *  @param wreq		A pointer to iwreq structure
 *  @return    		WLAN_STATUS_SUCCESS --success, otherwise fail
 */
static int
wlan_ieee_ps_mode_ioctl(wlan_private * priv, struct iwreq *wrq)
{
    wlan_adapter *Adapter = priv->adapter;
    int data;
    int ret = WLAN_STATUS_SUCCESS;
    ...
    if (!wrq->u.data.length) {
        ...
        if (data < 0 || data > IEEE_PS_MODE_PSPOLL) {
            PRINTM(INFO, "Wrong IEEE PS mode: %d\n", data);
            LEAVE();
            return -EINVAL;
        }
        if (Adapter->ieee_ps_mode != data) {
            Adapter->ieee_ps_mode = data;
            if (Adapter->PSState != PS_STATE_FULL_POWER) {
                wlan_exit_ps(priv, HostCmd_OPTION_WAITFORRSP);
            }
        }
 ...
 */
int
wlan_radio_ioctl(wlan_private * priv, u16 option)
{
    int ret = 0;
    wlan_adapter *Adapter = priv->adapter;
...
wlan_do_ioctl(struct net_device *dev, struct ifreq *req, int cmd)
{
    ...
    case WLAN_CUSTOM_IE_CFG:
        PRINTM(INFO, "Custom IE Ioctl\n");
        ret = wlan_custom_ie_ioctl(priv, req);
        break;
    ...
    case WLAN_SETINT_GETINT:
        ...
        case WLAN_ADHOC_G_PROT:
            ...
            break;
        case WLAN_DISABLE_AUTO_CHAN_SWITCH:
            ret = wlan_disable_auto_channel_switch(priv, wrq);
            break;
        }
        break;
...
#define TPC_DATA_NO_CHANG	0x7f
        ...
        case WLAN_11H_SET_QUIET_IE:
            ...
            break;
        case WLAN_CTS_POWER_CONTROL:
            ret = wlan_cts_power_ioctl(priv, wrq);
            break;
        case WLAN_IEEE_PS_MODE:
            ret = wlan_ieee_ps_mode_ioctl(priv, wrq);
            break;
        }
        break;
    ...
    case WLAN_SET_GET_2K:
        ...
        case WLAN_GET_MEM:
            ...
            break;
        case WLAN_READ_EEPROM:
            ret = wlan_read_eeprom(priv, wrq);
            break;
