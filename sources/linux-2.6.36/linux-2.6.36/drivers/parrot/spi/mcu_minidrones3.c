...
static char system_reset(struct mcu_minidrones3_data *drv_data)
{
	return mcu_sync_and_perform(drv_data, CMD_SYSTEM_RESET, 0, 0, NULL);
}
...
static struct mcu_minidrones3_data *reboot_drv_data;

static int sys_reset_notifier_block(struct notifier_block *this,
		unsigned long code, void *unused)
{
	if (code != SYS_RESTART)
		return NOTIFY_DONE;

	dev_info(&reboot_drv_data->spi->dev,
			"Requesting system reset from MCU\n");
	mutex_lock(&reboot_drv_data->iomutex);
	system_reset(reboot_drv_data);
	mutex_unlock(&reboot_drv_data->iomutex);
	mdelay(500);
	dev_warn(&reboot_drv_data->spi->dev, "We shouldn't be running!\n");
	return NOTIFY_DONE;
}

static struct notifier_block sys_reset_notifier = {
	.notifier_call = sys_reset_notifier_block
};

static void minidrones3_power_off(void)
{
	dev_info(&reboot_drv_data->spi->dev,
			"Requesting system power off from MCU\n");
	mutex_lock(&reboot_drv_data->iomutex);
	power_off(reboot_drv_data);
	mutex_unlock(&reboot_drv_data->iomutex);
	mdelay(500);
	dev_warn(&reboot_drv_data->spi->dev, "We shouldn't be running!\n");
}
...
static int __devinit mcu_minidrones3_probe(struct spi_device *spi)
{
	...
	reboot_drv_data = drv_data;
	pm_power_off = minidrones3_power_off;

	/* Register reboot notifier for power off and reboot */
	error = register_reboot_notifier(&sys_reset_notifier);
	if (error) {
		dev_err(&drv_data->spi->dev, "Failed to register reboot "
				"notifier: %d.\n", error);
		/* Non-fatal failure, just go on. */
	}
	return 0;
...
static int __devexit mcu_minidrones3_remove(struct spi_device *spi)
{
	...
	pm_power_off = NULL;
	unregister_reboot_notifier(&sys_reset_notifier);
