    ...
    switch (cmd)
    {
        ...
        *---------------------------------------------------------------------------*/
        ...
        case AAI_IOW_CODEC_I2S_FREQ:
            ...
            break;
        /*
        * Warning : In order to used ioctl ADC, audio channel must be already open.
        */
        case AAI_IOW_ADC_MONITORING_START:

            //Set ADC Mode
            aai_setbit(aai, AAI_ADC_MODES, AAI_ADC_MODES_MONITORING, 1);

	   //Start digitalization
	   aai_writereg(aai, 0x11, AAI_ADC_MONITOR);
           err = 0; 
           break;

        case AAI_IOR_ADC_MONITORING_DATA:            

	   //Set ADC Mode
	   aai_setbit(aai, AAI_ADC_MODES, AAI_ADC_MODES_MONITORING, 0);

	   //Get buffer
	   buf = aai_adc_getdata();
	   err = put_data(buf, 
		   (void __user *)arg,
		   sizeof(unsigned int) * AAI_NB_ADC);
